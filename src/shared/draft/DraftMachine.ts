...
import { generateTurnOrder } from "./turnOrders";
import { pickTwoAwakenings } from "../data/awakenings";
...
export class DraftMachine {
  ...
  pickMap(team: Team, mapId: string): DraftResult {
    ...
    if (mapBanMode === "bo1") {
      ...
      if (team === mapSelect) {
        this.state.mapBans.gameOrder[0] = mapId;
      } else if (team === sideSelect) {
        this.state.mapBans.gameOrder[1] = mapId;
      }
    }
